...
void LogicManager::update() {
    for (int i = 0; i < components.size(); i++) {
        for (int j = 0; j < components[i]->getUsedInputs(); j++) {
            components[i]->setInputValue(j, getPointValue(pair<int, int>(
                                             components[i]->getConnectedInputTo(j).first,
                                             components[i]->getConnectedInputTo(j).second)));
        }
        ...
        if (components[i]->getIsOutputUsed()) {
            if (components[i]->getConnectedOutputTo().first == 9) {
                ...
                mat[components[i]->getConnectedOutputTo().first][components[i]->getConnectedOutputTo().second] =
                        components[i]->getOutput();
            }
        }
    }
}
